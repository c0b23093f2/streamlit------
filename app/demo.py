...
def execute_trade(ticker: str, side: str, shares: int, price: float) -> tuple[bool, str]:
    ...
    if side == "買い":
        ...
        if pos.shares == 0:
            pos.cost_basis = 0.0
            # 全株売却したら自動売買の監視ラインも解除
            ss.get("auto_targets", {}).pop(ticker, None)
        # 自動売買のデイリーロスリミット集計にも実現損益を反映
        if "day_pnl" in ss:
            today = dt.date.today().isoformat()
            if ss["day_pnl"].get("date") != today:
...
def render_sidebar() -> None:
    ...
    with st.sidebar:
        st.markdown("### 📑 メニュー")
        try:
            st.page_link("pages/stock_detail.py", label="📊 銘柄詳細ページ")
            st.page_link("pages/auto_trade.py", label="🤖 自動売買")
        except Exception:
            pass
        st.divider()
        ...
        if st.button("↩️ 口座をリセット", use_container_width=True):
            # 自動売買側の状態（監視ライン・ログ・クールダウン・日次損益）も一緒に初期化
            for k in ("cash", "positions", "trades", "watchlist",
                      "auto_targets", "auto_log", "cooldown", "day_pnl"):
                ss.pop(k, None)
            init_state()
            st.rerun()
...
def render_positions() -> None:
    ...
    st.markdown("### 📦 保有ポジション")
    if not rows:
        st.info("保有ポジションはありません。チャート画面で銘柄を選択して注文してください。")
        return
...
def render_stock_detail() -> None:
    """銘柄詳細情報パネル"""
    st.markdown("### 📊 銘柄詳細")
    code = st.text_input("証券コード", key="detail_code", placeholder="例: 7203")
    ...
    st.caption(f"🔄 最終更新: {dt.datetime.now().strftime('%H:%M:%S')}")
...
def daily_rangebreaks(df: pd.DataFrame) -> list[dict]:
    """週末・祝日などデータのない日をチャートから除外（日足用）。"""
    try:
        ...
        return [dict(values=missing, dvalue=24 * 3600 * 1000)] if missing else []
    except Exception:
...
def render_chart() -> None:
    ...
    # 検索した銘柄を銘柄詳細ページ（stock_detail.py）で開く
    col2.write("")
    col2.write("")
    if col2.button("📊 銘柄詳細ページで開く", use_container_width=True, disabled=not ticker):
        st.session_state["detail_search"] = ticker.replace(".T", "")
        try:
            st.switch_page("pages/stock_detail.py")
        except Exception:
            st.warning("pages/stock_detail.py が見つかりません。demo.py と同じ階層に pages/ フォルダを配置してください。")

    # 期間選択をボタンで実装
    st.markdown("**チャート期間**")
    period_cols = st.columns(5)
    periods = ["1mo", "3mo", "6mo", "1y", "5y"]
    period_labels = ["1ヶ月", "3ヶ月", "6ヶ月", "1年", "5年"]

    if "chart_period" not in st.session_state:
        st.session_state.chart_period = "3mo"

    for idx, (period, label) in enumerate(zip(periods, period_labels)):
        if period_cols[idx].button(label, key=f"period_{period}", use_container_width=True):
    ...
    price = get_price(ticker)
    if price is None:  # 取得失敗時は直近終値でフォールバック（クラッシュ防止）
    ...
    if interval_map[period] == "1d":  # 週末・祝日のギャップを詰める（日足のみ）
        fig.update_xaxes(rangebreaks=daily_rangebreaks(df))
